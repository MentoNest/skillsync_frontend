...
const LargeToolCard: React.FC<LargeToolCardProps> = ({
  ...
  return (
    <div className={`relative overflow-hidden rounded-3xl p-8 text-white shadow-2xl transition-all duration-500 hover:scale-[1.02] hover:shadow-purple-500/20 group`}>
      ...
      {/* Decorative shapes */}
      <div
        className="absolute -right-10 -top-10 h-40 w-40 rounded-full bg-white/10 blur-3xl transition-transform duration-700 group-hover:translate-x-5 group-hover:-translate-y-5"
        aria-hidden="true"
      />
      <div
        className="absolute -left-10 -bottom-10 h-40 w-40 rounded-full bg-black/10 blur-3xl"
        aria-hidden="true"
      />
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="mt-auto">
          <button
            onClick={onClick}
            className="inline-flex items-center rounded-xl bg-white px-6 py-3 text-lg font-semibold text-purple-700 shadow-lg transition-all duration-300 hover:bg-purple-50 hover:px-8 active:scale-95 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-transparent"
