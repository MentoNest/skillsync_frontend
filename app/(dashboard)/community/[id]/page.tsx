...
export default function DiscussionDetailsPage() {
  ...
  const handleFollow = async () => {
    ...
    try {
      ...
      setFeedback(
        next ? "You are now following this user." : "You unfollowed this user.",
      );
    } catch {
      setDiscussion((prev) => (prev ? { ...prev, isFollowing: !next } : null));
  ...
  const handleShare = async () => {
    const shareUrl =
      typeof window !== "undefined"
        ? `${window.location.origin}/community/${discussion?.id}`
        : "";
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        ...
        {feedback ? (
          <p className="mb-4 text-sm text-cyan-700">{feedback}</p>
        ) : null}
