...
type ColorKey = "purple" | "teal" | "coral" | "pink" | "blue" | "green" | "amber" | "gray";
...
const COLOR_MAP: Record<ColorKey, ColorEntry> = {
...
function hashStringToColorKey(str: string): ColorKey {
  ...
  return COLOR_KEYS[index] as ColorKey;
}
...
export default function CategoryBadge({ category, color, className = "" }: CategoryBadgeProps) {
  const colorKey =
...
export function CategoryBadgeDemo() {
  ...
  return (
    ...
    >
      {categories.map((cat) => (
        <CategoryBadge key={cat} category={cat} color={undefined} />
      ))}
    </div>
  );
}
